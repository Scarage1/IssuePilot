...
class WebhookIssue(BaseModel):
    ...
    def label_names(self) -> list[str]:
        ...
        return [label.get("name", "") for label in self.labels if isinstance(label, dict)]
...
class IssueWebhookPayload(BaseModel):
    ...
    action: str = Field(..., description="The action that was performed (opened, edited, labeled, etc.)")
    issue: WebhookIssue
...
def get_webhook_config() -> WebhookConfig:
    ...
    return WebhookConfig(
        ...
        auto_analyze_on_open=os.getenv("WEBHOOK_AUTO_ANALYZE_OPEN", "true").lower() == "true",
        auto_analyze_on_edit=os.getenv("WEBHOOK_AUTO_ANALYZE_EDIT", "false").lower() == "true",
        auto_analyze_on_label=os.getenv("WEBHOOK_AUTO_ANALYZE_LABEL", "false").lower() == "true",
        required_label=os.getenv("WEBHOOK_REQUIRED_LABEL"),
        excluded_labels=os.getenv("WEBHOOK_EXCLUDED_LABELS", "").split(",") if os.getenv("WEBHOOK_EXCLUDED_LABELS") else [],
    )
