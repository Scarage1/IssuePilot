...
from .schemas import GitHubIssue, SimilarIssue
...
class DuplicateFinder:
    ...
    async def _get_embedding(self, text: str) -> List[float]:
        ...
        response = await self.client.embeddings.create(
            model="text-embedding-3-small", input=text[:8000]  # Limit input length
        )
    ...
    async def _get_embeddings_batch(self, texts: List[str]) -> List[List[float]]:
        ...
        for i in range(0, len(texts), batch_size):
            ...
            response = await self.client.embeddings.create(
                model="text-embedding-3-small", input=[t[:8000] for t in batch]
            )
    ...
    async def find_similar_issues(
        self, issue: GitHubIssue, existing_issues: List[Dict[str, Any]], top_k: int = 3
    ) -> List[SimilarIssue]:
        """
        ...
        existing_issues = [e for e in existing_issues if e["number"] != issue.number]
        ...
        if self.use_embeddings:
            ...
            similarities = self._compute_tfidf_similarity(target_text, existing_texts)
    ...
    async def _compute_embedding_similarity(
        self, target_text: str, comparison_texts: List[str]
    ) -> List[float]:
        """
    ...
    def check_exact_duplicate(
        self, issue: GitHubIssue, existing_issues: List[Dict[str, Any]]
