...
class TestAIEngineOpenAI:
    ...
    def test_init_with_api_key(self):
        ...
        with patch.dict('os.environ', {'OPENAI_API_KEY': 'test_key_that_is_long_enough_12345', 'AI_PROVIDER': 'openai'}):
            engine = AIEngine()
    ...
    def test_init_with_invalid_model(self):
        ...
        with patch.dict('os.environ', {'OPENAI_API_KEY': 'test_key_that_is_long_enough_12345', 'MODEL': 'invalid-model', 'AI_PROVIDER': 'openai'}):
...
class TestAIEngineGemini:
    ...
    def test_init_with_gemini_api_key(self):
        ...
        with patch.dict('os.environ', {'GEMINI_API_KEY': 'test_gemini_key_that_is_long_enough', 'AI_PROVIDER': 'gemini', 'MODEL': 'gemini-2.0-flash'}, clear=True):
            with patch('app.ai_engine.genai') as mock_genai:
                ...
                mock_genai.configure.assert_called_once_with(api_key="test_gemini_key_that_is_long_enough")

    def test_init_with_google_api_key_fallback(self):
        """Test initialization falls back to GOOGLE_API_KEY"""
        with patch.dict('os.environ', {'GOOGLE_API_KEY': 'test_google_key_that_is_long_enough', 'AI_PROVIDER': 'gemini', 'MODEL': 'gemini-2.0-flash'}, clear=True):
    ...
    def test_init_with_invalid_gemini_model(self):
        ...
        with patch.dict('os.environ', {'GEMINI_API_KEY': 'test_gemini_key_that_is_long_enough', 'AI_PROVIDER': 'gemini', 'MODEL': 'invalid-model'}):
            with patch('app.ai_engine.genai'):
                with pytest.raises(ModelError, match="Invalid model"):
                    AIEngine()
    ...
    def test_init_with_valid_gemini_models(self):
        ...
        for model in VALID_GEMINI_MODELS:
            with patch.dict('os.environ', {'GEMINI_API_KEY': 'test_gemini_key_that_is_long_enough', 'AI_PROVIDER': 'gemini'}):
                with patch('app.ai_engine.genai'):
                    engine = AIEngine(model=model)
                    assert engine.model == model
    ...
    def test_short_api_key_raises_error(self):
        ...
        with patch.dict('os.environ', {'GEMINI_API_KEY': 'short', 'AI_PROVIDER': 'gemini'}):
...
class TestAIEngineProviderSelection:
    ...
    def test_default_provider_is_openai(self):
        ...
        with patch.dict('os.environ', {'OPENAI_API_KEY': 'test_key_that_is_long_enough_12345'}, clear=True):
            engine = AIEngine()
            assert engine.provider == "openai"
    ...
    def test_provider_from_constructor(self):
        ...
        with patch.dict('os.environ', {'GEMINI_API_KEY': 'test_gemini_key_that_is_long_enough', 'MODEL': 'gemini-2.0-flash'}, clear=True):
            with patch('app.ai_engine.genai'):
